...
abstract class ControllerPaymentPaymill extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
            ...
            $data['base'] = preg_replace("/admin\/index\.php/", "", $this->request->server['SCRIPT_NAME']); //shoproot
        }
        ...
        $data['text_edit']  = $this->language->get('text_edit_'.$this->getPaymentName());
        $data['breadcrumbs'] = $this->getBreadcrumbs();
        $data['heading_title'] = $this->language->get('heading_title') . " (" . $this->getVersion() . ")";
        ...
        $data['entry_fast_checkout'] = $this->language->get('entry_fast_checkout');
        ...
        $data['entry_buttonSolution'] = $this->language->get('entry_buttonSolution');
        $data['entry_sepa_date'] = $this->language->get('entry_sepa_date');
        $data['entry_specific_creditcard'] = $this->language->get('entry_specific_creditcard');
        ...
        $data['paymill_privatekey'] = $this->getConfigValue($this->getPaymentName() . '_privatekey');

        if($this->getPaymentName() === 'paymillcreditcard') {
            $data['paymill_pci'] = $this->getConfigValue($this->getPaymentName() . '_pci');
        }
        ...
        $data['paymill_icon_visa'] = $this->getConfigValue($this->getPaymentName() . '_icon_visa');
        $data['paymill_icon_master'] = $this->getConfigValue($this->getPaymentName() . '_icon_master');
        $data['paymill_icon_amex'] = $this->getConfigValue($this->getPaymentName() . '_icon_amex');
        $data['paymill_icon_jcb'] = $this->getConfigValue($this->getPaymentName() . '_icon_jcb');
        $data['paymill_icon_maestro'] = $this->getConfigValue($this->getPaymentName() . '_icon_maestro');
        $data['paymill_icon_diners_club'] = $this->getConfigValue($this->getPaymentName() . '_icon_diners_club');
        $data['paymill_icon_discover'] = $this->getConfigValue($this->getPaymentName() . '_icon_discover');
        $data['paymill_icon_china_unionpay'] = $this->getConfigValue($this->getPaymentName() . '_icon_china_unionpay');
        $data['paymill_icon_dankort'] = $this->getConfigValue($this->getPaymentName() . '_icon_dankort');
        $data['paymill_icon_carta_si'] = $this->getConfigValue($this->getPaymentName() . '_icon_carta_si');
        $data['paymill_icon_carte_bleue'] = $this->getConfigValue($this->getPaymentName() . '_icon_carte_bleue');
        ...
        $this->response->setOutput($this->load->view('extension/payment/'. $this->getPaymentName(), $data));
    }
    ...
    protected function getBreadcrumbs()
    {
        ...
        $breadcrumbs[] = array(
            'href' => $this->url->link('extension/payment/' . $this->getPaymentName(), 'token=' . $this->session->data['token'], true),
    ...
    protected function validate()
    {
        ...
        if (!$this->user->hasPermission('modify', 'extension/payment/' . $this->getPaymentName())) {
        ...
        if (isset($this->request->post['paymill_differnet_amount'])) {
            if (!is_numeric($this->request->post['paymill_differnet_amount'])) {
                $data['error_warning'] = $this->language->get('error_different_amount');
                $validation = false;
            }
        }

        if (empty($publickey)) {
            $this->error['error_missing_publickey'] = $this->language->get('error_missing_publickey');
            $validation = false;
        }

        if (empty($privatekey)) {
            $this->error['error_missing_privatekey'] = $this->language->get('error_missing_privatekey');
            $validation = false;
        }
        return $validation;
    }
    ...
    protected function addPaymillWebhook($privateKey)
    {
        ...
        $url = $this->url->link('extension/payment/' . $this->getPaymentName() . '/webHookEndpoint');
