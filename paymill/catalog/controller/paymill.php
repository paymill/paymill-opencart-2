    ...
    Services_Paymill_LoggingInterface
{
    ...
    {
        global $config;
        $this->baseUrl = preg_replace("/\/index\.php/", "",
            $this->request->server['SCRIPT_NAME']);
        $this->load->model('checkout/order');
        $this->order_info = $this->model_checkout_order->getOrder(
            $this->session->data['order_id']);
        $amount = $this->currency->format($this->order_info['total'],
            $this->order_info['currency_code'], false, false);

        $data['paymill_amount'] = $amount;
        $data['paymill_currency'] = $this->order_info['currency_code'];
        $data['paymill_fullname'] = $this->order_info['firstname'] . ' ' .
            $this->order_info['lastname'];
        $data['paymill_css'] = $this->baseUrl .
            '/catalog/view/theme/default/stylesheet/paymill_styles.css';
        $data['paymill_iframe_css'] = $this->baseUrl .
            '/catalog/view/theme/default/stylesheet/paymill_iframe_styles.css';
        $data['paymill_image_folder'] = $this->baseUrl .
            '/catalog/view/theme/default/image/extension/payment';
        $data['paymill_js'] = $this->baseUrl .
            '/catalog/view/javascript/paymill/';
        $data['paymill_publickey'] = trim(
            $this->config->get($this->getPaymentName() . '_publickey'));
        $data['paymill_debugging'] = $this->config->get(
            $this->getPaymentName() . '_debugging');
        $data['paymill_buttonSolution'] = $this->config->get(
            $this->getPaymentName() . '_buttonSolution');
        $data['button_confirm'] = $this->language->get('button_confirm');

        $this->language->load('extension/payment/' . $this->getPaymentName());
        $data['paymill_accountholder'] = $this->language->get(
            'paymill_accountholder');
        $data['paymill_accountnumber'] = $this->language->get(
            'paymill_accountnumber');
        ...
        $data['paymill_description'] = $this->language->get(
            'paymill_description');
        $data['paymill_paymilllabel_cc'] = $this->language->get(
            'paymill_paymilllabel_cc');
        $data['paymill_paymilllabel_elv'] = $this->language->get(
            'paymill_paymilllabel_elv');
        ...
        $data['paymill_javascript_error'] = $this->language->get(
            'error_javascript');
        ...
        $data['paymill_icon_visa'] = $this->config->get(
            $this->getPaymentName() . '_icon_visa');
        $data['paymill_icon_master'] = $this->config->get(
            $this->getPaymentName() . '_icon_master');
        $data['paymill_icon_amex'] = $this->config->get(
            $this->getPaymentName() . '_icon_amex');
        $data['paymill_icon_jcb'] = $this->config->get(
            $this->getPaymentName() . '_icon_jcb');
        $data['paymill_icon_maestro'] = $this->config->get(
            $this->getPaymentName() . '_icon_maestro');
        $data['paymill_icon_diners_club'] = $this->config->get(
            $this->getPaymentName() . '_icon_diners_club');
        $data['paymill_icon_discover'] = $this->config->get(
            $this->getPaymentName() . '_icon_discover');
        $data['paymill_icon_china_unionpay'] = $this->config->get(
            $this->getPaymentName() . '_icon_china_unionpay');
        $data['paymill_icon_dankort'] = $this->config->get(
            $this->getPaymentName() . '_icon_dankort');
        $data['paymill_icon_carta_si'] = $this->config->get(
            $this->getPaymentName() . '_icon_carta_si');
        $data['paymill_icon_carte_bleue'] = $this->config->get(
        ...
        if (isset($payment['expire_month'])) {
            $payment['expire_month'] = $payment['expire_month'] <= 9 ? '0' .
                $payment['expire_month'] : $payment['expire_month'];
            $payment['expire_date'] = $payment['expire_month'] . "/" .
                $payment['expire_year'];
        } else {
        ...
        if ($this->getPaymentName() == "paymillcreditcard" &&
            ! $this->config->get($this->getPaymentName() . '_pci')) {
            ...
            return $this->load->view('extension/payment/paymill_pci_frame',
                $data);
        } else {
    ...
    private function showCreditcardIcons ()
    {
        ...
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_carta_si') ? 'carta-si' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_carte_bleue') ? 'carte-bleue' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_dankort') ? 'dankort' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_diners_club') ? 'diners-club' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_discover') ? 'discover' : '';
        $result[] = $this->config->get($this->getPaymentName() . '_icon_jcb') ? 'jcb' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_maestro') ? 'maestro' : '';
        $result[] = $this->config->get($this->getPaymentName() . '_icon_master') ? 'mastercard' : '';
        $result[] = $this->config->get(
            $this->getPaymentName() . '_icon_china_unionpay') ? 'china-unionpay' : '';
    ...
    public function confirm ()
    {
        ...
        if (empty($paymillToken)) {
            $this->log(
                "No paymill token was provided. Redirect to payments page.",
                '');
            ...
            $amountRaw = $this->currency->format($this->order_info['total'],
                $this->order_info['currency_code'], false, false);
            $amount = number_format($amountRaw, 2, '.', '') * 100;

            $source = $this->getVersion() . "_opencart_" . VERSION;
            $privateKey = trim(
                $this->config->get($this->getPaymentName() . '_privatekey'));
            ...
            if ($this->customer->getId() != null) {
                $table = $this->getDatabaseName();
                $row = $this->db->query(
                    "SELECT `clientId`, `paymentId` FROM $table WHERE `userId`=" .
                    $this->customer->getId());
                if ($row->num_rows === 1) {
                    ...
                    if (array_key_exists('email', $client)) {
                        if ($client['email'] !== $this->order_info['email']) {
                            ...
                            $this->log(
                                "Client-mail has been changed. Client updated",
            ...
            $this->log("Payment processing resulted in: ",
                ($result ? "Success" : "Fail"));
            ...
            if ($result === true) {
                ...
                $this->_saveUserData($this->customer->getId(),
                    $paymentProcessor->getClientId(),
                    $paymentProcessor->getPaymentId());
                $this->model_checkout_order->addOrderHistory(
                    $this->session->data['order_id'],
                    $this->config->get('config_order_status_id'), $comment,
                    true);

                $this->_updateOrderComment($this->session->data['order_id'],
                    $comment);

                $this->_saveOrderDetails($this->session->data['order_id'],
                    $transId, $preauthId);

                $this->response->redirect(
                    $this->url->link('checkout/success', '', true));
            } else {
                $responseCode = array_key_exists(
                    $paymentProcessor->getErrorCode(),
                    $this->_response_codes) ? $this->_response_codes[$paymentProcessor->getErrorCode()] : 'unknown error';
    ...
    private function _saveUserData ($userId, $clientId, $paymentId)
    {
        ...
        try {
            if ($userId != null) {
                ...
                if (! $dataAvailable) {
                    if ($this->config->get(
                        ...
                        $this->db->query(
                            "REPLACE INTO `$table` (`userId`, `clientId`, `paymentId`) VALUES($userId, '$clientId', '$paymentId')");
                    } else {
                        $this->db->query(
                            "REPLACE INTO `$table` (`userId`, `clientId`) VALUES($userId, '$clientId')");
                    }
                } else {
                    if ($this->config->get(
                        $this->getPaymentName() . '_fast_checkout')) {
                        $this->db->query(
                            "UPDATE `$table` SET `clientId`='$clientId', `paymentId`='$paymentId';");
                    } else {
                        $this->db->query(
                            "UPDATE `$table` SET `clientId`='$clientId';");
                    }
                }
            ...
            $this->log(
                "Error while saving Userdata: " . $exception->getMessage());
        }
    }
    ...
    private function _saveOrderDetails ($orderId, $transId, $preauthId)
    {
        ...
        $this->db->query(
            ...
            "pigmbh_paymill_orders` (`order_id`,`transaction_id`,`preauth_id`) VALUES ('" .
            $orderId . "', '" . $transId . "','" . $preauthId . "')");
    }
    ...
    private function _updateOrderComment ($orderId, $comment)
    {
        $result = $this->db->query(
            "SELECT `comment` FROM `" . DB_PREFIX .
            "order` WHERE `order_id`=" . $orderId);
        $this->db->query(
            "UPDATE `" . DB_PREFIX . "order` SET `comment`='" .
            $result->row['comment'] . "\n" . $comment .
    ...
    public function log ($message, $debuginfo)
    {
        if ($this->config->get($this->getPaymentName() . '_logging')) {
            $this->db->query(
                "INSERT INTO `" . DB_PREFIX .
                "pigmbh_paymill_logging` (`identifier`,`debug`,`message`) VALUES ('" .
                $this->_logId . "', '" .
                $this->db->escape($debuginfo) . "', '" .
    ...
    public function error ()
    {
        ...
        return $this->response->setOutput(
            $this->load->view('extension/payment/paymill_error', $data));
    }
    ...
    public function webHookEndpoint ()
    {
        ...
        $this->log('WebHookValidation',
            var_export($this->validateNotification($request), true));
        $this->log('WebHookBody', var_export($request, true));

        if ($this->validateNotification($request)) {
            $order_new_status = '11'; // status refunded according to
            // vanilla-installation
            $order_id = $this->getOrderIdFromNotification(
                $request['event']['event_resource']['transaction']['description']);
        ...
        if (isset($notification) && ! empty($notification)) {
            ...
            if (isset($notification['event']['event_type'])) {
                if ($notification['event']['event_type'] == 'refunded.succeeded') {
                    $id = null;
                    if (isset(
                        $notification['event']['event_resource']['transaction']['id'])) {
                    ...
                    $transactionObject = new Services_Paymill_Transactions(
                        $privateKey, Metadata::PAYMILL_API);
