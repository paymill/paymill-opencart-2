		...
		.ready(
				function() {
					...
					$('#paymill_card_number')
							.keyup(
									function() {
										$("#paymill_card_number")[0].className = $("#paymill_card_number")[0].className
												.replace(
														/paymill-card-number-.*/g,
														'');
										var cardnumber = $(
												'#paymill_card_number').val();
										var detector = new BrandDetection();
										var brand = detector.detect(cardnumber);
										if (brand !== 'unknown'
												&& $.inArray(brand,
														PAYMILL_BRAND) !== -1) {
											$('#paymill_card_number').addClass(
													"paymill-card-number-"
															+ brand);
...
function validate() {
	...
	var message = '';
	if (PAYMILL_PAYMENT === "paymillcreditcard") {
		if (paymill.cardType($('#paymill_card_number').val()).toLowerCase() === 'maestro'
				&& (!$('#paymill_card_cvc').val() || $('#paymill_card_cvc')
						.val() === "000")) {
		...
		if (!paymill.validateExpiry($("#paymill_card_expiry_date").val().split(
				"/")[0], $("#paymill_card_expiry_date").val().split("/")[1])) {
		...
		if (isSepa()) {
			...
			if ($('#paymill_bic').val().length !== 9
					&& $('#paymill_bic').val().length !== 11) {
	...
	if (error) {
		...
		if (PAYMILL_TRANSLATION.bridge.hasOwnProperty(error.apierror)) {
			alert("API returned error:"
					+ PAYMILL_TRANSLATION.bridge[error.apierror]);
		...
		$(".checkout-heading").children('a :last').click(); // click on step5
		// Modify
	} else {
