...
require_once dirname(dirname(dirname(dirname(__FILE__)))) .
         '/paymill/lib/Services/Paymill/LoggingInterface.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))) .
         '/paymill/lib/Services/Paymill/Preauthorizations.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))) .
         '/paymill/lib/Services/Paymill/Transactions.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))) .
         '/paymill/lib/Services/Paymill/Refunds.php';
require_once dirname(dirname(dirname(dirname(__FILE__)))) .
        ...
        Services_Paymill_LoggingInterface
{
    ...
    protected function _getBreadcrumbs ()
    {
        ...
        $breadcrumbs[] = array(
                'href' => $this->url->link('sale/order/info', 
                        'token=' . $this->session->data['token'] . $orderIdUrl, 
                        true),
    ...
    public function index ()
    {
        ...
        $data['text_payment_method'] = $this->language->get(
                'text_payment_method');
        ...
        if ($order_info) {
            ...
            $data['data_order_total'] = $this->currency->format(
                    $order_info['total'], $order_info['currency_code'], 
                    $order_info['currency_value']);
            $data['data_order_date_added'] = date(
        ...
        $url_capture = $this->url->link('custom/paymillOrder/capture', '', 
                'SSL');
        $data['url_capture'] = $url_capture . '&token=' .
                 $this->session->data['token'] . '&orderId=' . $orderId;
        
        $url_refund = $this->url->link('custom/paymillOrder/refund', '', 'SSL');
        $data['url_refund'] = $url_refund . '&token=' .
                 $this->session->data['token'] . '&orderId=' . $orderId;
        ...
        return $this->response->setOutput(
                $this->load->view('custom/paymillOrder', $data));
    }
    ...
    private function proceedCapture ($preauth_id)
    {
        ...
        if (is_array($preauth)) {
            ...
            try {
                ...
                $this->log('Capture successfully', 
                        $this->paymillProcessor->getTransactionId());
                $this->saveTransactionId($orderId, 
                        $this->paymillProcessor->getTransactionId());
                $orderStatusId = $this->db->query(
                        'SELECT `order_status_id` FROM `' . DB_PREFIX .
                                 'order_status` WHERE `name`= "Complete"')->row['order_status_id'];
    ...
    private function proceedRefund ($transactionId)
    {
        ...
        $this->log('Transaction used for Refund', 
                var_export($transaction, true));
        if (is_array($transaction)) {
            try {
                $result = $this->paymillRefund->create(
                        array(
                ...
                $orderStatusId = $this->db->query(
                        'SELECT `order_status_id` FROM `' . DB_PREFIX .
                                 'order_status` WHERE `name`= "Refunded"')->row['order_status_id'];
                $this->addOrderHistory($orderId, 
                        array(
                                'order_status_id' => $orderStatusId,
                                'notify' => true,
    ...
    private function addOrderHistory ($order_id, $data)
    {
        ...
        $this->db->query(
                "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" .
                         ...
                         "', date_modified = NOW() WHERE order_id = '" .
                         (int) $order_id . "'");
        
        $this->db->query(
                "INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" .
                         (int) $order_id . "', order_status_id = '" .
                         (int) $data['order_status_id'] . "', notify = '" .
                         (isset($data['notify']) ? (int) $data['notify'] : 0) .
                         "', comment = '" .
                         $this->db->escape(strip_tags($data['comment'])) .
                         "', date_added = NOW()");
        
        $order_info = $this->model_sale_order->getOrder($order_id);
        
        // Send out any gift voucher mails
        if ($this->config->get('config_complete_status_id') ==
        ...
        if ($data['notify']) {
            ...
            $order_status_query = $this->db->query(
                    "SELECT * FROM " . DB_PREFIX .
                             "order_status WHERE order_status_id = '" .
                             (int) $data['order_status_id'] .
                             "' AND language_id = '" .
                             (int) $order_info['language_id'] . "'");
            ...
            if ($data['comment']) {
                ...
                $message .= strip_tags(
                        html_entity_decode($data['comment'], ENT_QUOTES, 
                                'UTF-8')) . "\n\n";
            }
    ...
    private function getOrderDetails ($orderId)
    {
        ...
        $result = $this->db->query(
                'SELECT * FROM `' . DB_PREFIX . 'pigmbh_paymill_orders` ' .
                         $where);
    ...
    private function saveTransactionId ($orderId, $id)
    {
        ...
        $this->db->query(
                'UPDATE `' . DB_PREFIX .
                         'pigmbh_paymill_orders` SET `transaction_id` = "' .
                         $this->db->escape($id) . '" ' . $where);
    }
